$(function(){
	...
	//CONSULTA 1
	//MUESTRA LAS IMAGENES POR GEOLOCALIZACION DEL USUARIO: QUE TENGAN LATITUD Y LONGITUD.
	//ADEMAS COMPUEBAN QUE TENGAN INFORMACION RELATIVA A LA GEOLOCACION Y CUANTA AREA VAMOS A MIRAR CON CENTRO LAS COORDENADAS DADAS.
	$("#query1btn").click(function(){
		...
		*/
		...
		vLat = $("#query1Lattext").val();
		vLon = $("#query1Lontext").val();
				
        //Realiza la busqueda en flickr		
		$.getJSON('https://api.flickr.com/services/rest/?method=flickr.photos.search&api_key=' 
		+ api_key + '&user_id=' + user_id + '&lat=' + vLat + '&lon=' + vLon + '&format=json&nojsoncallback=1',
		{
			accuracy: 11,
			has_geo: 1,
            format: "json",
	...
	//CONSULTA 4
	//MUESTRA LAS IMAGENES CON MAYOR O IGUAL NUMERO DE VISITAS PROPORCIONADOS.
	$("#query4btn").click(function(){		
		...
		*/
		...
		////Realiza la busqueda en flickr
		//extras: views, numero de visitas.
		
        $.getJSON('https://api.flickr.com/services/rest/?method=flickr.people.getPhotos&api_key=' 
		+ api_key + '&user_id=' + user_id + '&nojsoncallback=1',
        {
            extras: views=$("#query4text").val(), 
            format: "json",
        },
		function(info){
			var i;
	...
	//CONSULTA 5
	//MUESTRA LAS IMAGENES DEL USUARIO DEPENDIENDO DEL TIPO DE CONTENIDO: PHOTOS, SCREENSHOT, ETC
	...
	//CONSULTA 6
	//MUESTRA LAS IMAGENES POR ETIQUETAS. sEGUN LAS PALABRAS CLAVES ASOCIADAS A LA IMAGEN.
